...
describe('The move() function', () => {
    it('Action.DAY_EARLIER should return one day before', () => {
        const meeting: Meeting = { title: 'Javascript Course', date: new Date(2022, 11, 26, 12), duration: 2 };
        expect(move(meeting, Action.DAY_EARLIER).date.getDate()).to.be.equal(25);
    });
    ...
    it('Action.HOUR_LATER at 20:00 should raise an error', () => {
        const meeting: Meeting = { title: 'Javascript Course', date: new Date(2022, 11, 26, 20), duration: 2 };
...
describe('The Timetable class', () => {
    it('timetable.put adds a meeting', () => {
        ...
        timetable.put({ title: 'Javascript Course', date: new Date(2022, 11, 26, 12), duration: 2 });
        expect(timetable.getMeetingsLength()).to.be.equal(1);
    });
    it('timetable.put on a taken timeslot returns false', () => {
        const timetable: Timetable = new Timetable();
        timetable.put({ title: 'Javascript Course', date: new Date(2022, 11, 26, 12), duration: 2 });
        expect(timetable.put({ title: 'Security Course', date: new Date(2022, 11, 26, 13), duration: 2 })).to.be.false;
    });
    it('timetable.get gets an added meeting', () => {
        const timetable: Timetable = new Timetable();
        const date: Date = new Date(2022, 11, 26, 12);
        timetable.put({ title: 'Javascript Course', date: date, duration: 2 });
        expect(timetable.get(date).title).to.be.equal('Javascript Course');
    });
    ...
    it("timetable.perform alters timetable's meetings", () => {
        const timetable: Timetable = new Timetable();
        timetable.put({ title: 'Javascript Course', date: new Date(2022, 11, 26, 12), duration: 2 });
        timetable.put({ title: 'Security Course', date: new Date(2022, 11, 26, 17), duration: 2 });
        const actions: Array<Action> = [
            Action.DAY_EARLIER,
            Action.HOUR_LATER
        ];
        timetable.perform(actions);
        expect(timetable.get(new Date(2022, 11, 25, 12)).title).to.be.equal('Javascript Course');
        expect(timetable.get(new Date(2022, 11, 26, 18)).title).to.be.equal('Security Course');
    });
});
