...
app.get('/login', async function (req, res) {
  ...
  let s = await student.find({},"name");
  let arr = [];
  for(let i = 0; i<s.length; i++){
    arr.push(s[i].name);
  }
  str = arr.join([separator = ','])
  res.render('login.ejs',{students: arr});
});
...
app.post('/register', async function (req, res) {     
    ...
    let data = new student(item);
    data.save();
    let c = await course.find({});

    for(var j =0; j < 7; j++){
      await addCourseToStudent(data._id, c[j]);
      await addStudentToCourse(c[j]._id, data);
    }
    res.redirect("/login");
...
app.get('/grade', async function (req, res){
  ...
  str = arr.join([separator = ','])
  res.render('grade.ejs',{students: arr});
});
...
app.post('/grade', async function (req, res){
    ...
    let g =  await createGrade({vote: grade});
    let s = await student.find({name: n});
    let c = await course.find({name: cour});
    await AddGradeToStudent(s[0]._id, g);
    await AddGradeToCourse(c[0]._id,g);
